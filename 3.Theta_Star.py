...
class Node:
    def __init__(self, position, g, h, parent=None, direction=None):
...
def is_line_of_sight_clear(start, end, maze):
    x0, y0 = start
    x1, y1 = end
    dx = abs(x1 - x0)
    dy = abs(y1 - y0)
...
def theta_star(maze, start, target, screen):
    open_set = []
    ...
    while open_set:
        ...
        for neighbor in neighbors:
            ...
            # Check if line of sight exists to parent node
            if current.parent and is_line_of_sight_clear(current.position, neighbor, maze):
                # No need for an extra g cost to go through the parent
            ...
            if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                ...
                if not any(neighbor == node.position for node in open_set):
                    heapq.heappush(open_set, Node(neighbor, tentative_g_score, heuristic(neighbor, target), current))
...
def draw_maze(maze, path, screen, open_set, closed_set, current_position):
    ...
    if current_position:
        ...
        rect = pygame.Rect(y * CELL_SIZE, x * CELL_SIZE, CELL_SIZE, CELL_SIZE)
        pygame.draw.rect(screen, GREEN, rect)  # Current node (green)
...
path = theta_star(maze, start, target, screen)
end_time = time.time()
...
output_path = os.path.join(output_dir, "Theta_Star.png")
